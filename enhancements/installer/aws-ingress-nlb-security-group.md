...
- Migrate to use LBC as the default on CIO.
...
- Create `install-config.yaml` enabling the use of SG and `lbType=NLB` (already exists).
- The installer creates a security group to be used by the ingress controller during the InfraReady phase.
- The installer generates the CIO manifests: enabling LB type NLB and passing Security Group Names (IDs are not known yet during the manifest phase).
- CIO creates the service for the default router, filling in the annotations for NLB and SG IDs (conversion to ID is required before setting the annotation).
- CCM provisions the Load Balancer NLB with the security group, updating SGs with the required rules for ingress (based on listeners) and egress (based on service and health check ports).
